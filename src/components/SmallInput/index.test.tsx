...
describe('SmallInput component', () => {
  it('renders properly', () => {
    const { getByPlaceholderText, queryByTestId } = render(<SmallInput placeholder="custom-input" />);
  ...
  it('renders error icon and message when error prop is not falsy', () => {
    const { getByTestId, getByText, queryByTestId } = render(
      <SmallInput placeholder="custom-input" error="error description" />
    );
  ...
  it('renders check icon when success prop is true', () => {
    const { getByTestId, queryByTestId } = render(<SmallInput placeholder="custom-input" success={true} />);
    expect(queryByTestId(/warning-icon/i)).not.toBeInTheDocument();
  ...
  it('renders error when both error and success are truthy', () => {
    const { getByTestId, getByText, queryByTestId } = render(
      <SmallInput placeholder="custom-input" success={true} error="error description" />
    );
    expect(getByTestId(/warning-icon/i)).toBeInTheDocument();
    expect(queryByTestId(/check-icon/i)).not.toBeInTheDocument();
    expect(getByText(/error description/i)).toBeInTheDocument();
  ...
  it('renders children', () => {
    const { getByText } = render(
      <SmallInput placeholder="custom-input">
        <span>children</span>
      </SmallInput>
    );
