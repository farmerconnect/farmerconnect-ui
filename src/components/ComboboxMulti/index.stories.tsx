...
Default.args = {
  ...
  secondItemRender: (content: any) => {
    const id = content.id.split(':').length > 1 ? content.id.split(':').pop().split('.').pop() : content.id;

    const productId =
      content.product_id.split(':').length > 1 ? content.product_id.split(':').pop().split('.')[1] : content.product_id;

    const companyPrefix =
      content.product_id.split(':').length > 1 ? content.product_id.split(':').pop().split('.')[0] : content.product_id;
    return (
      <Fragment>
        <b>ID: {id}</b>
        <p>
          Product id: {productId} ︱ Company prefix: {companyPrefix}
        </p>
      </Fragment>
    );
  },
