import { HTMLProps, ReactNode } from 'react';
import { Label } from './styles';

export type RadioProps = {
  children?: ReactNode;
} & HTMLProps<HTMLInputElement>;

const Radio = ({ children, className, style, ...props }: RadioProps) => (
  <Label className={className} style={style} disabled={props.disabled}>
    <input type="radio" {...props} />
    <RadioChecked className="checked" />
    <RadioUnchecked className="unchecked" />
    {children}
  </Label>
);

export default Radio;

const RadioUnchecked = (props: { className: string }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
    <rect x="4.5" y="4.5" width="15" height="15" rx="7.5" stroke="currentColor" />
  </svg>
);

const RadioChecked = (props: { className: string }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
    <rect x="4.5" y="4.5" width="15" height="15" rx="7.5" fill="currentColor" stroke="currentColor" />
    <path
      d="M10.1348 14.1591L8.03182 12.0561C7.91859 11.9426 7.76486 11.8788 7.60455 11.8788C7.44423 11.8788 7.2905 11.9426 7.17727 12.0561C6.94091 12.2924 6.94091 12.6742 7.17727 12.9106L9.71061 15.4439C9.94697 15.6803 10.3288 15.6803 10.5652 15.4439L16.9773 9.03181C17.2136 8.79545 17.2136 8.41363 16.9773 8.17726C16.864 8.06378 16.7103 8 16.55 8C16.3897 8 16.236 8.06378 16.1227 8.17726L10.1348 14.1591Z"
      fill="white"
    />
  </svg>
);
