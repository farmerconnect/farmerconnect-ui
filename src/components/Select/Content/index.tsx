...
const renderSingle = (
  ...
  const searchIsVisible = searchOptions.visible;
  return (
    <Container>
      {searchIsVisible && (
        <ContainerSearch>
          <Search
            placeholder={searchOptions.textSearch}
            value={searchValue}
          ...
          {!!content.length ? (
            content.map((item: any, index: number) => (
              <ItemMultiple
                key={index}
                content={item}
                contentRender={itemRender(item)}
                limitReached={limitReached}
                handleSelected={(e: any, item: any) => {
                  itemSelect(e, item);
                }}
              />
            ))
