...
const Container = styled.button<IButtonProps>`
  background-color: ${(props) =>
    props.backgroundColor || props.theme?.button?.colors?.primary || buttonDefaultStyles.colors.primary};
  border: 0px;
  border-radius: 12px;
  ...
  &:hover {
    background-color: ${(props) =>
      props.hoverColor || props.theme?.button?.colors?.tertiary || buttonDefaultStyles.colors.tertiary};
  }
